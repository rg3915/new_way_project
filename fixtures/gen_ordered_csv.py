...
with io.open('fixtures/pedidos.csv', 'wt') as f:
    f.write(
        u'id,customer,employee,vehicle,kit_optional,dealership,kiosk,status,created_at,modified_at\n')
    for i in range(repeat):
        customer = random.randint(1, 200)
        employee = random.randint(2, 100)
        ...
        ordered_list.append((i + 1, customer, employee, vehicle,
                             kit_optional, dealership, kiosk, status, created_at, date))
